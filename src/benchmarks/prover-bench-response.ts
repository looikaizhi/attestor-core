...
import type { ProtocolResult, ProverKind } from '#src/benchmarks/prover-bench.ts'
import {
...
const DEFAULT_CONFIG: SweepConfig = {
	...
	bandwidthMbps: +(process.env.BENCH_BANDWIDTH_MBPS ?? 200),
	latencyMs: +(process.env.BENCH_LATENCY_MS ?? 10),
	requestSizeBytes: +(process.env.BENCH_REQUEST_BYTES ?? 2048),
	responseSizesBytes: (process.env.BENCH_RESPONSE_SIZES ?? '1024,2048,4096,8192,16384,32768,65536,131072')
		.split(',')
		.map(s => +s)
	...
	outputCsv: process.env.BENCH_CSV ?? join('benchmarks', 'prover-bench-response.csv'),
...
function printSummary(results: SweepResult[]) {
	const table = results.map(result => ({
		responseSize: `${result.responseSizeBytes} B`,
...
async function sweepResponseSizes(config: SweepConfig) {
	logger.info(
		{
			iface: config.iface,
			...
			responseSizesBytes: config.responseSizesBytes,
			tlsVersion: config.tlsVersion,
			zkEngine: config.zkEngine,
		},
		'starting response-size sweep'
	)
	...
	for(const responseSize of config.responseSizesBytes) {
		for(let i = 0; i < 10; i++) {
			await cleanupIptablesCounters(ports, config.useSudo)
			await installIptablesCounters(ports, config.useSudo)

			await setNetem(config.iface, config.bandwidthMbps, config.latencyMs, config.useSudo)
			if(config.showQdisc) {
			...
			try {
				...
				logger.error({ err, responseSize }, 'prover run failed')
			} finally {
			...
			logger.info({
				...
				error,
				run: i
			}, 'finished one response-size run')
		}
	}
...
async function main() {
	await sweepResponseSizes(DEFAULT_CONFIG)
}

main().catch(err => {
	logger.error({ err }, 'response-size sweep failed')
	process.exit(1)
})
